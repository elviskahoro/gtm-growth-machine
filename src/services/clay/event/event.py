...
class EventAttendee(BaseModel):
    ...
    def etl_get_file_name(
        ...
        Args:
            extension (str): The file extension to use. Defaults to ".jsonl".

        Returns:
            str: The generated file name with source, timestamp, and identifier.
        """
        # Use email if available, otherwise fall back to name
        identifier = self.email or self.name
        if identifier:
            # Clean the identifier for filesystem compatibility using sanitize_string
            cleaned_identifier = sanitize_string(identifier.replace("@", "·"))
...
def test_event_attendee_model_creation() -> None:
    ...
    minimal_attendee = EventAttendee(
        source="webinar",
        created_at=datetime(2024, 3, 15, 10, 30, 0, tzinfo=timezone.utc),
    )
