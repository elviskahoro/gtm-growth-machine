...
class Webhook(BaseModel):
    ...
    def etl_get_bucket_name() -> str:
        return "chalk-ai-devx-marketplace-product-descriptions"
    ...
    def lance_get_table_name() -> str:
        return "marketplace_product_descriptions"
    ...
    def lance_get_schema() -> pa.Schema:
        return pa.schema(
            [
                ...
                pa.field("product_category_id", pa.string()),
                pa.field("hid", pa.int32()),
                pa.field("product_category_hid", pa.int32()),
    ...
    def etl_is_valid_webhook(
        ...
        return (
            ...
            and self.hid > 0
            and self.product_category_hid > 0
        )
    ...
    def etl_get_invalid_webhook_error_msg(
        ...
        if len(self.id.strip()) == 0:
            errors.append("Empty product ID")

        if len(self.title.strip()) == 0:
            errors.append("Empty product title")

        if len(self.description.strip()) == 0:
            errors.append("Empty product description")

        if self.hid <= 0:
            errors.append("Invalid hid")

        if self.product_category_hid <= 0:
            errors.append("Invalid product_category_hid")
